...
std::vector<std::string> ccxLoadsSurfaceTractionModifyCommand::get_syntax_help()
{
  ...
  help[0] = "ccx "; 
  help[0].append("modify surfacetraction <surfacetraction_id> ");
...
bool ccxLoadsSurfaceTractionModifyCommand::execute(CubitCommandData &data)
{
  ...
  //if (!ccx_iface.modify_loadssurfacetraction(surfacetraction_id, options, options_marker, options2, options3))
  {
    output = "Failed!\n";
    PRINT_ERROR(output.c_str());
    return false;
